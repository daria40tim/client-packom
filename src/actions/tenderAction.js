...
import { FULLCOSTS_FAIL, FULLCOSTS_REQUEST, FULLCOSTS_SUCCESS, GET_TENDER_FILTER_DATA_FAIL, GET_TENDER_FILTER_DATA_REQUEST, GET_TENDER_FILTER_DATA_SUCCESS, TENDER_DECIDE_FAIL, TENDER_DECIDE_REQUEST, TENDER_DECIDE_SUCCESS, TENDER_FAIL, TENDER_LIST_FAIL, TENDER_LIST_REQUEST, TENDER_LIST_SORTEDBY_DATE, TENDER_LIST_SORTEDBY_STATUS, TENDER_LIST_SORTEDBY_TENDER_ID, TENDER_LIST_SORTEDBY_TZ_ID, TENDER_LIST_SORT_FAIL, TENDER_LIST_SORT_SUCCESS, TENDER_LIST_SUCCESS, TENDER_REQUEST, TENDER_SUCCESS } from "../constants/tenderConstants"

export const listTenders = (s_date, e_date, projs, tz_ids, tender_sts) => async(dispatch) => {
    try {
...
export const tenderFilterData = () => async(dispatch) => {
  try {

    const userInfo = localStorage.getItem('userInfo') ? JSON.parse(localStorage.getItem('userInfo')) : null
    dispatch({type: GET_TENDER_FILTER_DATA_REQUEST})
    let auth = "Bearer " + userInfo.token
    ...
    dispatch({
      type: GET_TENDER_FILTER_DATA_FAIL,
      payload: error.response && error.response.data.message ? error.response.data.message : error.message
    })
  }
}